...
async function main() {
	...
	// Populate the PostGIS `location` column from latitude/longitude for all newly created rows.
	// This uses PostGIS functions; the DB must have the extension enabled.
	...
	console.log(
		`Seeding finished. Created ${data.length} reports around Frankfurt (±${radiusMeters} m) within ±24h.`
	);
}
